...
`CollisionGroup` is a (case-sensitive) built-in component name that assigns an Instance to a collision group.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via `Instance.CollisionGroup: string`.
