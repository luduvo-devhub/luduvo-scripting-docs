...
`Admin` is a (case-sensitive) built-in component name that marks an identity as an administrator.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via `Instance:GetIdentity().isAdmin`.
