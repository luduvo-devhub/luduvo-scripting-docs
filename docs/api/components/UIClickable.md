...
`UIClickable` is a (case-sensitive) built-in component name that opts a UI element into pointer hit-testing.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via `Instance.Activated: Signal`.
