...
`AmbientLight` is a (case-sensitive) built-in component name that stores the world's ambient-light settings.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via `game.Lighting.Ambient`.
