...
`SurfaceAppearance` is a (case-sensitive) built-in component name that stores an Instance's surface texture assets.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via `Instance.TextureId`, `EmissiveTextureId`, and `UnderlayTextureId`.
...
| `albedo` | `u64 asset ID` |  |
| `normal` | `u64 asset ID` |  |
| `metallic_roughness` | `u64 asset ID` |  |
| `occlusion` | `u64 asset ID` |  |
| `emissive` | `u64 asset ID` |  |
| `underlay` | `u64 asset ID` |  |
