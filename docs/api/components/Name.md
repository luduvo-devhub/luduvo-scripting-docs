...
`Name` is a (case-sensitive) built-in component name that stores the name shown in Studio's Outliner.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via `Instance.Name: string`.
...
| --- | --- | --- |
| `id` | `u32` | Interned string ID; shown as Name |
