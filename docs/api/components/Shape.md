...
`Shape` is a (case-sensitive) built-in component name that selects an Instance's primitive shape.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via `Instance.Kind: string`.
