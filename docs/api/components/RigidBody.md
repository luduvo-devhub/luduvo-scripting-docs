...
`RigidBody` is a (case-sensitive) built-in component name that stores an Instance's physical material properties.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via `Instance.Density`, `Friction`, `Restitution`, `LinearDamping`, and `AngularDamping`.
