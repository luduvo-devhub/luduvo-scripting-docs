...
`CharacterAnimation` is a (case-sensitive) built-in component name that opts an Instance into character animation.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via the Instance animation methods.
...
This is a [tag component](index.md#tag-components){ data-preview } and has no stored fields.
