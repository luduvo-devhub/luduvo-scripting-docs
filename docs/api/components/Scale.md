...
`Scale` is a (case-sensitive) built-in component name that stores an Instance's size.
...
As a [Value-Based Component](index.md#value-based-components){ data-preview }, Queries expose `query.Scale[i]` as `vector` (read/write). It can also be accessed via `Instance.Size: vector`.
