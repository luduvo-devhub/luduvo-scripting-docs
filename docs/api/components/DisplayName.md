...
`DisplayName` is a (case-sensitive) built-in component name that stores a user's display name identifier.
...
While this component can be used as a filter in Queries, it currently does not expose any of its fields to Queries. However, it can be accessed via `Instance:GetIdentity().displayName`.
...
| --- | --- | --- |
| `id` | `u32` | Interned string ID; editor label: NameId |
